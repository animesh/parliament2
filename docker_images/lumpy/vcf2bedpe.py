...
def vcfToBedpe(vcf_file, bedpe_out):
    ...
    for line in vcf_file:
        ...
        if var.info['SVTYPE'] != 'BND':
            ...
            span = list(map(int, var.info['CIPOS'].split(',')))
            s1 = b1 + span[0] - 1
            e1 = b1 + span[1]

            span = list(map(int, var.info['CIEND'].split(',')))
            s2 = b2 + span[0] - 1
            e2 = b2 + span[1]
            ...
            for i in range(len(sample_list)):
                s = sample_list[i]
                gt = v[9 + i]
                format_dict = dict(list(zip(format_list, gt.split(':'))))
            ...
            bedpe_out.write('\t'.join(list(map(str,
                                          [var.chrom,
                                           s1,
                                           e1,
                                           var.chrom,
            ...
            if mate_id in unmatched:
                ...
                span = list(map(int, prim.info['CIPOS'].split(',')))
                s1 = b1 + span[0] - 1
                e1 = b1 + span[1]

                span = list(map(int, sec.info['CIPOS'].split(',')))
                s2 = b2 + span[0] - 1
                e2 = b2 + span[1]
